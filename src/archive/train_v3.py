...
EXPERIMENT_DIR  = os.path.expanduser('~/F3Net/experiments/f3net_c40_v3')

# ---------------------------------------------------------------------------
# Hyperparameters
# ---------------------------------------------------------------------------
BATCH_SIZE   = 12
ACCUM_STEPS  = 10       # batch effectif = 12 * 10 = 120
MAX_EPOCHS   = 5
FRAME_NUM    = 270
LOG_FREQ     = 40       # en micro-steps
EVAL_FREQ    = 10       # évaluation toutes les 1/10 d'epoch
GPU_IDS      = [0]
...
def main():
    ...
    logger.info(f'Optimizer      : Adam  lr={lr_scaled}  (0.0002 × {ACCUM_STEPS})')
    logger.info(f'Batch micro    : {BATCH_SIZE}  |  Accum: {ACCUM_STEPS}  '
                f'|  Batch effectif: {BATCH_SIZE * ACCUM_STEPS}')
    ...
    logger.info(f'Steps per epoch (micro): {steps_per_epoch}')
    logger.info(f'Optim steps per epoch  : {steps_per_epoch // ACCUM_STEPS}')

    # ------------------------------------------------------------------
    # Modèle
    # ------------------------------------------------------------------
    model = Trainer(GPU_IDS, PRETRAINED_PATH, accum_steps=ACCUM_STEPS)

    # Compteurs séparés : micro-steps et vrais optimizer steps
    micro_step  = 0   # total de micro-batchs vus
    optim_step  = 0   # total de vrais optimizer.step() effectués
    best_auc    = 0.0
    ...
    for epoch in range(MAX_EPOCHS):
        ...
        step = 0   # micro-steps dans l'epoch courante
        while step < steps_per_epoch:
            ...
            loss, did_step = model.optimize_weight()
            ...
            if micro_step % LOG_FREQ == 0:
                logger.info(
                    f'  micro_step {micro_step:6d}  '
                    f'optim_step {optim_step:5d}  '
                    f'loss: {loss:.4f}'
                )
            ...
            if step % eval_every == 0:
                ...
                logger.info(
                    f'  [Val  @ epoch {epoch} micro_step {step}] '
        ...
        # Fin d'epoch : applique les gradients résiduels si nécessaire
        model.flush()
        logger.info(
            f'  Fin epoch {epoch} — '
            f'micro_steps: {micro_step}  optim_steps: {optim_step}'
        )
