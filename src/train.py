...
EXPERIMENT_DIR  = os.path.expanduser('~/F3Net/experiments/f3net_c40')
...
MAX_EPOCHS  = 5
FRAME_NUM   = 270
LOG_FREQ    = 40       # log loss every N steps
EVAL_FREQ   = 10       # evaluate every 1/EVAL_FREQ of an epoch

GPU_IDS     = [0]      # set to [0,1] if two GPUs are available
...
def main():
    ...
    print("Taille des datasets (Real / Fake) :", len(dataset_real), len(dataset_fake))
    print("Taille des dataloaders (Real / Fake) :", len(dataloader_real), len(dataloader_fake))
    print("================================")	
    ...
    for epoch in range(MAX_EPOCHS):
        ...
        while step < steps_per_epoch:
            ...
            if model.total_steps % LOG_FREQ == 0:
                logger.info(f'  step {model.total_steps:5d}  loss: {loss:.4f}')
